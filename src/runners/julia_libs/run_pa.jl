...
include("../gscaling/proxalg/proxalg_util.jl")
include("../gscaling/proxalg/proxalg_pg.jl")
include("../gscaling/proxalg/proxalg_panoc.jl")
include("../gscaling/proxalg/proxalg_zerofpr.jl")
include("../gscaling/proxalg/proxalg_panocplus.jl")
...
# ============================================================
# ProximalAlgorithms.jl parameters
# ============================================================

proxalg_theta_bound = 20.0
proxalg_project_f_evals = false
proxalg_cache_digits = 6

verbose_proxalg = false

proxalg_maxiters = 20
proxalg_tol = 1e-4

proxalg_gamma = 1e-1
proxalg_adaptive = false
proxalg_minimum_gamma = 1e-12
proxalg_max_backtracks = 5
...
function _proxalg_common_kwargs(theta0_calib; maxiters, tol)
    return (
        J1 = Int[],
        J0 = Int[],
        theta0 = theta0_calib,
        ...
        theta_bound = proxalg_theta_bound,
        project_f_evals = proxalg_project_f_evals,
        ...
        maxiters = maxiters,
        tol = tol,
        cache_digits = proxalg_cache_digits,

        verbose = verbose_proxalg,
    )
end
...
for s in s_vals
    ...
    # ============================================================
    # ProximalAlgorithms.jl ForwardBackward / proximal gradient
    # f(theta) = DDGFact+_Upsilon(theta)
    # g(theta) = indicator of box [-theta_bound, theta_bound]
    # ============================================================

    if run_method(:proxalg_pg)
        runtime_by_method[:proxalg_pg] = @elapsed begin
            result_proxalg_pg = calibrate_upsilon_proxalg_pg_ddfactplus(
                Csym,
                s,
                t;
                _proxalg_common_kwargs(
                    theta0_calib;
                    maxiters = proxalg_pg_maxiters,
                    tol = proxalg_pg_tol,
                )...,

                gamma = proxalg_pg_gamma,
                adaptive = proxalg_pg_adaptive,
                minimum_gamma = proxalg_pg_minimum_gamma,
                reduce_gamma = proxalg_pg_reduce_gamma,
                increase_gamma = proxalg_pg_increase_gamma,
            )
        end

        obj_by_method[:proxalg_pg] = result_proxalg_pg.obj
    end


    # ============================================================
    # ProximalAlgorithms.jl PANOC
    # ============================================================

    if run_method(:proxalg_panoc)
        runtime_by_method[:proxalg_panoc] = @elapsed begin
            result_proxalg_panoc = calibrate_upsilon_proxalg_panoc_ddfactplus(
                Csym,
                s,
                t;
                _proxalg_common_kwargs(
                    theta0_calib;
                    maxiters = proxalg_maxiters,
                    tol = proxalg_tol,
                )...,

                gamma = proxalg_gamma,
                adaptive = proxalg_adaptive,
                minimum_gamma = proxalg_minimum_gamma,
                max_backtracks = proxalg_max_backtracks,
            )
        end

        obj_by_method[:proxalg_panoc] = result_proxalg_panoc.obj
    end


    # ============================================================
    # ProximalAlgorithms.jl ZeroFPR
    # ============================================================

    if run_method(:proxalg_zerofpr)
        runtime_by_method[:proxalg_zerofpr] = @elapsed begin
            result_proxalg_zerofpr =
                calibrate_upsilon_proxalg_zerofpr_ddfactplus(
                    Csym,
                    s,
                    t;
                    _proxalg_common_kwargs(
        ...
        obj_by_method[:proxalg_zerofpr] = result_proxalg_zerofpr.obj
    end


    # ============================================================
    # ProximalAlgorithms.jl PANOCplus
    # ============================================================

    if run_method(:proxalg_panocplus)
        runtime_by_method[:proxalg_panocplus] = @elapsed begin
            result_proxalg_panocplus =
                calibrate_upsilon_proxalg_panocplus_ddfactplus(
                    Csym,
                    s,
                    t;
                    _proxalg_common_kwargs(
        ...
        obj_by_method[:proxalg_panocplus] = result_proxalg_panocplus.obj
    end
