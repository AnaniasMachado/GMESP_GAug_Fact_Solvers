...
run_tag = "projection_free_greedy_dual_gap"

mkpath("results")

results_filepath =
    "results/results_$(run_tag)_n$(n)_kappa$(kappa).csv"
...
function _print_pf_summary(label::Symbol, result)
    ...
    println(
        "$(label) adaptive_max_iter = ",
        result.adaptive_max_iter,
    )
    println("$(label) min_iter = ", result.min_iter)
    println(
        "$(label) iteration_power = ",
        result.iteration_power,
    )
...
function _pf_primal_dual_gaps(
    ...
    # Measure only the runtime of the exact greedy algorithm
    # that computes upsilon, nu, eta, rho, tau, and alpha for
    # the fixed Theta.
    dual_runtime = @elapsed begin
        sol_G =
    ...
    dual_obj = dual_obj_data.objective_value
...
for s in s_vals
    ...
    duality_gap_by_method = Dict{Symbol,Float64}()
    dual_runtime_by_method = Dict{Symbol,Float64}()
    ...
    theta0_calib =
        _needs_theta0() ? _make_theta0_calib(n) : nothing
    ...
    if run_method(:custom_bfgs)
        ...
        obj_by_method[:custom_bfgs] =
            result_custom_bfgs.obj
    end
    ...
    if run_method(:ppa_one)
        runtime_by_method[:ppa_one] = @elapsed begin
            result_ppa_one =
                calibrate_upsilon_ppa_ddfactplus(
                    Csym,
                    s,
                    t;
                    J1 = Int[],
                    ...
                    knitro_maxtime_real =
                        ppa_knitro_maxtime_real,
                    knitro_algorithm =
                        ppa_knitro_algorithm,
                    knitro_bar_murule =
                        ppa_knitro_bar_murule,
                    knitro_honorbnds =
                        ppa_knitro_honorbnds,
    ...
    if run_method(:pf_lmo_lmo)
        runtime_by_method[:pf_lmo_lmo] = @elapsed begin
            result_pf_lmo_lmo =
                calibrate_upsilon_projection_free_ddfactplus(
                    Csym,
                    s,
                    t;
        ...
        obj_by_method[:pf_lmo_lmo] =
            eval_pf_lmo_lmo.primal_obj

        gap_by_method[:pf_lmo_lmo] =
            eval_pf_lmo_lmo.primal_gap

        dual_gap_by_method[:pf_lmo_lmo] =
            eval_pf_lmo_lmo.dual_gap

        duality_gap_by_method[:pf_lmo_lmo] =
            eval_pf_lmo_lmo.duality_gap

        dual_runtime_by_method[:pf_lmo_lmo] =
            eval_pf_lmo_lmo.dual_runtime
    ...
    if run_method(:pf_lmo_po)
        ...
        obj_by_method[:pf_lmo_po] =
            eval_pf_lmo_po.primal_obj

        gap_by_method[:pf_lmo_po] =
            eval_pf_lmo_po.primal_gap

        dual_gap_by_method[:pf_lmo_po] =
            eval_pf_lmo_po.dual_gap

        duality_gap_by_method[:pf_lmo_po] =
            eval_pf_lmo_po.duality_gap

        dual_runtime_by_method[:pf_lmo_po] =
            eval_pf_lmo_po.dual_runtime
    ...
    if run_method(:pf_po_lmo)
        ...
        obj_by_method[:pf_po_lmo] =
            eval_pf_po_lmo.primal_obj

        gap_by_method[:pf_po_lmo] =
            eval_pf_po_lmo.primal_gap

        dual_gap_by_method[:pf_po_lmo] =
            eval_pf_po_lmo.dual_gap

        duality_gap_by_method[:pf_po_lmo] =
            eval_pf_po_lmo.duality_gap

        dual_runtime_by_method[:pf_po_lmo] =
            eval_pf_po_lmo.dual_runtime
    ...
    for m in selected_methods
        if haskey(obj_by_method, m) &&
           !haskey(gap_by_method, m)
            gap_by_method[m] =
                obj_by_method[m] - z_ls
        end
    end
    ...
    # Order:
    #   n, s, t,
    #   all ordinary reformulation gaps,
    #   projection-free LP dual and duality gaps,
    #   projection-free dual runtimes,
    #   all method runtimes.
    # ============================================================
    ...
    # Preserve the gaps of every existing reformulation.
    for m in selected_methods
    ...
    # Add the LP-based dual gap, primal-dual gap, and dual
    # construction runtime only for projection-free solutions.
    for m in selected_pf_methods
        if haskey(dual_gap_by_method, m)
            row[Symbol("$(m)_dual_gap")] =
                dual_gap_by_method[m]

            row[Symbol("$(m)_duality_gap")] =
                duality_gap_by_method[m]

            row[Symbol("$(m)_dual_runtime")] =
                dual_runtime_by_method[m]
        end
    end

    for m in selected_methods
        if haskey(runtime_by_method, m)
            row[Symbol("$(m)_runtime")] =
                runtime_by_method[m]
        end
    end
    ...
    @printf(
        "  local_search_runtime = %.2fs\n",
        runtime_ls,
    )

    for m in selected_methods
        if haskey(gap_by_method, m) &&
           haskey(runtime_by_method, m)
            _print_method_line(
                m,
                gap_by_method,
                runtime_by_method,
    ...
    for m in selected_pf_methods
        if haskey(dual_gap_by_method, m)
            @printf(
                "  %-22s dual_gap = % .6e    duality_gap = % .6e    dual_runtime = %.6fs\n",
                string(m),
                dual_gap_by_method[m],
                duality_gap_by_method[m],
                dual_runtime_by_method[m],
...
# Keep every original reformulation-gap column.
for m in selected_methods
    push!(
        gap_cols,
        Symbol("$(m)_gap"),
...
# Additional columns only for projection-free calibration.
for m in selected_pf_methods
    push!(
        pf_dual_cols,
        Symbol("$(m)_dual_gap"),
    ...
    push!(
        pf_dual_cols,
        Symbol("$(m)_duality_gap"),
    )

    push!(
        pf_dual_cols,
        Symbol("$(m)_dual_runtime"),
    )
end
