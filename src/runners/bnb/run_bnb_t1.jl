...
results_filepath =
    "results/bnb_t1_ddgfactplus_primal_fixing_data$(data_n)_n$(n)_s2_to_$(n - 1)_heap.csv"
...
println("="^82)
println("GMESP specialized B&B test: DDGFactplus, t = 1, closed-form node bound")
...
# ============================================================
# Run t = 1 DDGFactplus B&B for s = 2, ..., n - 1
# ============================================================
results = []
...
for s in 2:(n - 1)
    println()
    println("-"^82)
    println("Running DDGFactplus t = 1 B&B")
    println("s:                              $s")
    println("fixing_rule:                    $fixing_rule")
    flush(stdout)
    ...
    relaxation_out =
        _symbol_string_stat(st, :relaxation, "DDGFactplus")

    method_out =
        _symbol_string_stat(st, :method, "bnb_t1_closed_form")

    solver_used_out =
        _symbol_string_stat(st, :solver_used, "t1_closed_form_bnb")
